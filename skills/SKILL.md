...
```bash
cd /path/to/workspace/skills/github-trending
npm init -y
npm install playwright
npx playwright install chromium  # browser for any JS-rendered pages
```

Note: If `npx playwright install --with-deps` fails (needs root/sudo), run `npx playwright install chromium` without deps. GitHub trending is server-rendered so no deps needed.
